...
describe('joinOpenAICompatibleUrl', () => {
  it('normalizes slashes between the base url and path', () => {
    expect(joinOpenAICompatibleUrl('https://llm.yunhaoli.top/v1', '/models')).toBe(
      'https://llm.yunhaoli.top/v1/models',
    );
    expect(joinOpenAICompatibleUrl('https://llm.yunhaoli.top/v1/', 'chat/completions')).toBe(
      'https://llm.yunhaoli.top/v1/chat/completions',
    );
  });
});
